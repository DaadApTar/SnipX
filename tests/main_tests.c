...
bool test_dynamic_circular_array() {
  ...
  assert_int(&test, 21, array.capacity);
  assert_int(&test, 0, dynamic_circular_array_push(&array, (void *)cdata2, strlen(cdata2) + 1));
  assert_int(&test, 39, array.capacity);
  assert_int(&test, 0, dynamic_circular_array_push(&array, &uldata3, sizeof(uldata3)));
  assert_int(&test, 47, array.capacity);
  assert_int(&test, 0, dynamic_circular_array_push(&array, &uldata3, sizeof(uldata3)));
  assert_int(&test, 52, array.capacity);
  assert_int(&test, 0, dynamic_circular_array_push(&array, &uldata3, sizeof(uldata3)));
  assert_int(&test, 52, array.capacity);
  assert_int(&test, 0, dynamic_circular_array_push(&array, (void *)cdata2, strlen(cdata2) + 1));
  assert_int(&test, 52, array.capacity);
  ...
  void *got_data;
  printf("------Getting data------\n");
  got_data = dynamic_circular_array_get(&array, array.next_index-4, &size);
  assert_int(&test, sizeof(uldata3), size);
  assert_int(&test, uldata3, *(unsigned long *)got_data);
  free(got_data);
  got_data = dynamic_circular_array_get(&array, array.next_index-3, &size);
  assert_int(&test, sizeof(uldata3), size);
  assert_int(&test, uldata3, *(unsigned long *)got_data);
  free(got_data);
  got_data = dynamic_circular_array_get(&array, array.next_index-2, &size);
  assert_int(&test, sizeof(idata1), size);
  assert_int(&test, idata1, *(int *)got_data);
  free(got_data);
  got_data = dynamic_circular_array_get(&array, array.next_index-1, &size);
  ...
  assert_int(&test, 0, dynamic_circular_array_push(&array, (void *)cdata2, strlen(cdata2) + 1));
  assert_int(&test, 71, array.capacity);
  assert_int(&test, 0, dynamic_circular_array_push(&array, (void *)cdata2, strlen(cdata2)));
  assert_int(&test, 80, array.capacity);
  assert_int(&test, 0, dynamic_circular_array_push(&array, (void *)cdata2, strlen(cdata2) + 1));
  assert_int(&test, 94, array.capacity);
  assert_int(&test, 0, dynamic_circular_array_push(&array, (void *)cdata4, strlen(cdata4) + 1));
  printf("-----Final size test-----\n");
